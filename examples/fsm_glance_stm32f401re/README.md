# LSM6DSV16X Glance and Deglance Gesture Detection on STM32F401RE Nucleo-64 (JSON-Configured FSM)
...
The program uses the `lsm6dsv16x` Rust driver and applies a JSON configuration (`GLANCE_DETECTION`) to program the sensor's FSM. The example runs on the STM32F401RE with interrupts configured on PB4 to signal FSM events.
...
*This README provides a detailed explanation of the embedded Rust program for glance and deglance gesture detection on STM32F401RE using the LSM6DSV16X sensor and JSON-generated FSM configuration.*
