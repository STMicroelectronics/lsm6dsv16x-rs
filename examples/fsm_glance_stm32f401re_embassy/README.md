...
This project demonstrates how to interface the **LSM6DSV16X** accelerometer sensor with an **STM32F401RE** microcontroller board. The program configures the sensor FSM using a JSON file converted to Rust code, detects gesture events, and outputs notifications over UART.
...
- The sensor is configured by applying a sequence of register writes and delays generated by the **`ucf-tool`** from a JSON file (`lsm6dsv16x_glance_detection.ucf`), which programs the sensor's FSM for gesture detection.
