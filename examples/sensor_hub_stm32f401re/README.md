# LSM6DSV16X Sensor Hub with LIS2MDL Magnetometer and LPS22DF Barometer on STM32F401RE Nucleo
...
The code is written in Rust using the `stm32f4xx-hal` hardware abstraction layer and the `lsm6dsv16x`, `lis2mdl`, and `lps22df` sensor driver crates.
...
### Sensor Hub Mode (Mode 2) Details

- The LSM6DSV16X operates in sensor hub mode (mode 2), enabling connection of up to 4 external sensors via its I2C master interface.
...
- Pass-through mode allows direct access to external sensor registers via the main I2C interface for configuration.
