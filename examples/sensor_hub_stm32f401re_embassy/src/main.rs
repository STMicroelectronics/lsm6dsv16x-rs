...
async fn main(_spawner: Spawner) {
    ...
    let master = Lsm6dsv16xMaster::from_bus(I2cBus::new(i2c, lsm6dsv16x_addr as u8), delay.clone());
    ...
    let mut lps22df = lps22df::Lps22df::new_bus(lsm6dsv16x_for_lps22df, delay.clone());

    // Check device ID
    match master.borrow_mut().device_id_get() {
        Ok(id) => {
            if id != lsm6dsv16x::ID {
                writeln!(&mut msg, "Device (LSM6DSV16X) ID mismatch: {:#02x}", id).unwrap();
        ...
        for _ in 0..num {
            ...
            match f_data.tag {
                ...
                Tag::TimestampTag => {
                    ...
                    writeln!(&mut msg, "Timestamp {:.1} [us] (lsb: {})", ts_usec, ts).unwrap();
                    let _ = tx.blocking_write(msg.as_bytes());
                    msg.clear();
                }
                ...
                _ => {
                    writeln!(&mut msg, "Invalid TAG: {:x}", f_data.tag as u8).unwrap();
                    let _ = tx.blocking_write(msg.as_bytes());
                    msg.clear();
                }
            }
