...
async fn main(_spawner: Spawner) {
    ...
    sensor.tap_detection_set(tap).unwrap();

    // Set wake-up thresholds and duration
    let thr = TapThresholds { x: 0, y: 0, z: 3 };
    sensor.tap_thresholds_set(thr).unwrap();
    ...
    sensor.tap_time_windows_set(tap_win).unwrap();
    sensor.tap_mode_set(TapMode::BothSingleDouble).unwrap();

    // Set Output Data Rate
    sensor.xl_data_rate_set(Odr::_480hz).unwrap();
    // Set full scale
    sensor.xl_full_scale_set(XlFullScale::_8g).unwrap();
    ...
    loop {
        ...
        if status.single_tap == 1 {
            writeln!(&mut msg, "Single TAP").unwrap();
            let _ = tx.blocking_write(msg.as_bytes());
            msg.clear();
        }
        if status.double_tap == 1 {
            writeln!(&mut msg, "Double TAP").unwrap();
            let _ = tx.blocking_write(msg.as_bytes());
            msg.clear();
        }
    }
