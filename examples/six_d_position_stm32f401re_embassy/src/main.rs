...
async fn main(_spawner: Spawner) {
    ...
    // On the Nucleo FR401 the interrupt pin is connected to pin PB4.
    ...
    sensor.filt_sixd_feed_set(FiltSixdFeed::LowPass).unwrap();
    sensor.threshold_6d_set(SixDThreshold::_60deg).unwrap();
    ...
    loop {
        ...
        if status.six_d == 1 {
            ...
            writeln!(&mut msg, "{}", message).unwrap();
            let _ = tx.blocking_write(msg.as_bytes());
            msg.clear();
        }
    }
